...
const WebGISMap = () => {
  ...
  // Fonction pour sauvegarder un nouveau gisement
  const handleSaveDeposit = async (depositData, geojsonFeature) => {
    try {
      console.log("Gisement créé avec succès:", depositData);
      ...
      console.error("Erreur lors de l'ajout du gisement:", error);
      alert(
  ...
  const loadDepositsFromAPI = async () => {
    try {
      ...
      console.warn('Backend indisponible, utilisation des données de démonstration');
  ...
  const handleLayerEdit = (layer) => {
    ...
    console.log('Édition de la couche:', layer);
  };
  ...
  const handleLayerClick = (layer, feature, event) => {
    console.log('Clic sur la couche:', layer, feature);
  };

  const handleLayerLoad = (layer, leafletLayer) => {
    console.log('Couche chargée:', layer.name);
  };

  const handleLayerError = (layer, error) => {
    console.error('Erreur de couche:', layer.name, error);
  };
