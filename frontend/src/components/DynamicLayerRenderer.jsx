...
const DynamicLayerRenderer = ({ 
  ...
  const createPopupContent = useCallback((layer) => {
    const stats = [];
    if (layer.area_km2) stats.push(`Superficie: ${parseFloat(layer.area_km2).toFixed(2)} km²`);
    if (layer.length_km) stats.push(`Longueur: ${parseFloat(layer.length_km).toFixed(2)} km`);
    if (layer.point_count) stats.push(`Points: ${layer.point_count}`);

    return `
      <div class="geospatial-popup">
        <h3 class="popup-title">${layer.name}</h3>
        ...
        <div class="popup-details">
          ...
          <div class="popup-row">
            <strong>Statut:</strong> <span class="status-${layer.status}">${layer.status}</span>
          </div>
          <div class="popup-row">
            <strong>Format:</strong> ${layer.source_format}
          </div>
          <div class="popup-row">
            <strong>Géométrie:</strong> ${layer.geometry_type}
          </div>
          ${stats.length > 0 ? `
          ...
          <div class="popup-date">
            Créé le: ${new Date(layer.created_at).toLocaleDateString('fr-FR')}
          </div>
        </div>
      </div>
    `;
  ...
  const createLeafletLayer = useCallback(async (layer) => {
    try {
      ...
      console.error(`Erreur lors de la création de la couche ${layer.name}:`, error);
      if (onLayerError) {
    ...
    for (const layer of visibleLayers) {
      if (!layersRef.current.has(layer.id)) {
        try {
          ...
          console.error(`Erreur lors du chargement de la couche ${layer.name}:`, error);
        }
      }
    }
  ...
  const fitBounds = useCallback(() => {
    ...
    try {
      ...
      console.warn('Impossible de centrer sur les couches:', error);
    }
  }, [map]);
