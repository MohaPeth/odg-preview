...
const LayersManagementTable = ({ 
  ...
  const fetchLayers = useCallback(async (showLoader = true) => {
    ...
    try {
      ...
      console.warn('Backend géospatial indisponible, utilisation des couches de démonstration', err);
    ...
    try {
      ...
      if (result.success) {
        ...
        setLayers(prev => prev.map(l => 
          l.id === layer.id ? { ...l, is_visible: !l.is_visible } : l
        ));
        
        // Notifier le parent
        if (onLayerToggle) {
          onLayerToggle(layer.id, !layer.is_visible);
        }
      } else {
  ...
  const TypeBadge = ({ type }) => {
    ...
    if (!option || option.value === 'all') return type;
          ...
          <Table>
            ...
            <TableBody>
              {layers.length === 0 ? (
                ...
                layers.map((layer) => (
                  <TableRow key={layer.id} className="hover:bg-gray-50">
                    <TableCell>
                      <Checkbox
                        checked={selectedRows.has(layer.id)}
                        onCheckedChange={(checked) => handleRowSelect(layer.id, checked)}
                      />
                    </TableCell>
                    <TableCell>
                      <Button
                        ...
                        <div className="grid grid-cols-2 gap-2 text-xs text-gray-600">
                          <div className="flex items-center space-x-1">
                            ...
                            <Badge variant="outline" className="text-xs px-1 py-0">
                              {layer.file_name?.split('.').pop()?.toUpperCase() || 'N/A'}
                            </Badge>
                          </div>
                          
                          <div className="flex items-center space-x-1">
                            <MapPin className="h-3 w-3 text-gray-400" />
                            <span>{layer.features_count || 0} éléments</span>
                          </div>
                        </div>
                        
                        <div className="text-xs text-gray-500 flex items-center space-x-1">
                          <Calendar className="h-3 w-3" />
                          <span>Créé le {formatDate(layer.created_at)}</span>
                        </div>
      ...
      <Dialog open={deleteDialog.open} onOpenChange={(open) => setDeleteDialog({ open, layer: null })}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Êtes-vous sûr de vouloir supprimer la couche "{deleteDialog.layer?.name}" ?
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
                            ...
                            <div>
                              ...
                              <div className="mt-1 flex items-center space-x-1">
                                ...
                                <span>{layer.features_count || 0}</span>
                              </div>
                            </div>
                            ...
                            <div>
                              ...
                              <div className="mt-1 flex items-center space-x-1">
                                ...
                                <span>{formatDate(layer.created_at)}</span>
                              </div>
                            </div>
