...
# Détection du dialecte SQL pour gérer PostGIS
USE_POSTGIS = False
try:
    from geoalchemy2 import Geometry
    from geoalchemy2.shape import to_shape
    USE_POSTGIS = True
    logger.info("PostGIS disponible - Support géométries activé")
except ImportError:
    logger.info("PostGIS non disponible - Mode SQLite standard")
...
class MiningDeposit(db.Model):
    ...
    def update_geometry(self):
        """Met à jour la colonne géométrie PostGIS depuis lat/lon (PostgreSQL uniquement)"""
        # Pour SQLite, cette méthode ne fait rien
        # Pour PostgreSQL avec PostGIS, il faudra ajouter la colonne geom via migration
        pass
    ...
    def to_geojson_feature(self):
        ...
        try:
            ...
            logger.error(f"Erreur conversion GeoJSON deposit {self.id}: {e}")
            return None
...
class ExploitationArea(db.Model):
    ...
    def update_geometry(self):
        """Met à jour la colonne géométrie PostGIS depuis coordinates JSON (PostgreSQL uniquement)"""
        # Pour SQLite, cette méthode ne fait rien
        pass
    ...
    def to_geojson_feature(self):
        """Conversion en Feature GeoJSON pour export cartographique"""
        try:
            coords = json.loads(self.coordinates) if isinstance(self.coordinates, str) else self.coordinates
            ...
            logger.error(f"Erreur conversion GeoJSON area {self.id}: {e}")
            return None
...
class Infrastructure(db.Model):
    ...
    def to_geojson_feature(self):
        ...
        try:
            ...
            logger.error(f"Erreur conversion GeoJSON infrastructure {self.id}: {e}")
            return None
