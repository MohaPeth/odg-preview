...
# Utiliser JSON pour SQLite, JSONB pour PostgreSQL
try:
    from sqlalchemy.dialects.postgresql import dialect as PostgreSQLDialect
    JSON_TYPE = JSONB  # PostGIS utilise JSONB
except:
    JSON_TYPE = JSON  # SQLite utilise JSON
...
class GeospatialLayer(db.Model):
    ...
    style_config = db.Column(JSON, nullable=False, default=dict, server_default='{}')
    ...
    layer_metadata = db.Column(JSON, nullable=False, default=dict, server_default='{}')
    ...
    # Timestamps (fuseau horaire local)
    ...
    def to_dict(self):
        ...
        # Récupérer le nombre de features depuis les métadonnées si non défini
        feature_count = self.point_count
        if not feature_count and self.layer_metadata:
            feature_count = self.layer_metadata.get('source_info', {}).get('feature_count', 0)
        
        # Extraire le GeoJSON des métadonnées pour le frontend
        geojson_data = None
        if self.layer_metadata and 'geojson' in self.layer_metadata:
            geojson_data = self.layer_metadata['geojson']
        
        # Vérifier si la géométrie PostGIS existe
        has_geometry = self.geom is not None
        ...
        return {
            ...
            'geojson': geojson_data,  # GeoJSON pour le frontend
            'geometry': has_geometry,  # Booléen indiquant la présence de géométrie
            'areaKm2': self.area_km2,
            'lengthKm': self.length_km,
            'pointCount': feature_count,
    ...
    def to_geojson_feature(self):
        ...
        # Si le GeoJSON complet est stocké dans les métadonnées (mode JSON pur)
        if self.layer_metadata and 'geojson' in self.layer_metadata:
            geojson_data = self.layer_metadata['geojson']
            
            # Si c'est une FeatureCollection, retourner la première feature enrichie
            if geojson_data.get('type') == 'FeatureCollection':
                features = geojson_data.get('features', [])
                if features:
                    first_feature = features[0].copy()
                    # Enrichir avec les métadonnées de la couche
                    first_feature['properties']['layerId'] = self.id
                    first_feature['properties']['layerName'] = self.name
                    first_feature['properties']['layerType'] = self.layer_type
                    return first_feature
                # Si FeatureCollection vide, retourner quand même (ne pas essayer Shapely)
                return None
            elif geojson_data.get('type') == 'Feature':
                feature = geojson_data.copy()
                feature['properties']['layerId'] = self.id
                feature['properties']['layerName'] = self.name
                return feature
            # Si métadonnées GeoJSON présentes mais structure inconnue, retourner tel quel
            return geojson_data
        
        # Mode PostGIS classique avec Shapely - ne devrait être utilisé que pour les anciennes couches
        if not self.geom:
            return None
